...
#	ObjF = scip.quicksum(T[j,k]*z[i,j,k] for i in xrange(max_len-1) for j in xrange(n_clusts) for k in xrange(n_clusts))
	ObjF = scip.quicksum(-3*dup[j] for j in xrange(n_terms))+scip.quicksum(simvec[j]*x[i,j] for i in xrange(max_len) for j in xrange(n_terms)) +scip.quicksum(3*T[j,k]*z[i,j,k] for i in xrange(max_len-1) for j in xrange(n_clusts) for k in xrange(n_clusts))
