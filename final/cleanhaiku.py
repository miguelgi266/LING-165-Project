...
#			if pos[i,t2n[tag]] != 0:print n2w[i]
	...
	ObjF = scip.quicksum(-3*dup[j] for j in xrange(n_terms))#duplicates penalty
	ObjF += scip.quicksum(simvec[j]*x[i,j] for i in xrange(max_len) for j in xrange(n_terms))#similarity score
#	ObjF += scip.quicksum(3*T[j,k]*z[i,j,k] for i in xrange(max_len-1) for j in xrange(n_clusts) for k in xrange(n_clusts))#co-occurence reward
